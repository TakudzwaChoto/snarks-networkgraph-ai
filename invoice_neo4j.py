...
class neo4j:
    ...
    def create_monitoring_nodes(self, data):
        ...
        for d in data:
            # 创建一个监测点节点
            monitoring_node = Node('Stackholder', id=d['RCH'], 
                                   WEC=d['Cs'],
                                   RECORD=d['K'])
            self.g.create(monitoring_node)
            monitoring_nodes[d['RCH']] = monitoring_node
        return monitoring_nodes            

    def create_rels_between_nodes_and_monitoring(self, nodes, monitoring_nodes):
        ...
        for node_id, node in nodes.items():
            if node_id in monitoring_nodes:
                ...
                if node_id not in management_relations:
                    # 创建一个管理关系
                    management_relation = Relationship(monitoring_node, "MANAGES", node)
    ...
    def export_nodes_and_rels(self):
        ...
        relationships_list = [{"source": str(rel["source"]), "target": str(rel["target"]), "type": rel["type"]} for rel in relationships]
...
if __name__ == "__main__":
    ...
    monitoring_nodes = handler.create_monitoring_nodes(data)
    handler.create_rels_between_nodes_and_monitoring(nodes, monitoring_nodes)
